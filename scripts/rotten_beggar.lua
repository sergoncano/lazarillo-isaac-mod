...
function LazarilloMod:stealFromRottenBeggar(entityBeggar, collider)  
    ...
    if math.floor(math.random(0, hitChance)) == 0 then
        ...
        beggarSprite:Load("gfx/beggars/rotten_beggar/rotten_beggar.anm2", true)  
...
function LazarilloMod:triggerStealFromRottenBeggar(entityBeggar, collider, low)	  
    local beggarSprite = entityBeggar:GetSprite()
    ...
    if (not (beggarSprite:IsPlaying("PayPrize") or beggarSprite:IsPlaying("Prize") or beggarSprite:IsPlaying("PayNothing"))) then
        ...
        if playerType == lazaroType then
            ...
            if ((not beggarSprite:IsPlaying("GetStolen")) and notTeleporting)  then
                return LazarilloMod:stealFromRottenBeggar(entityBeggar, collider)            
...
LazarilloMod:AddCallback(ModCallbacks.MC_PRE_SLOT_COLLISION, LazarilloMod.triggerStealFromRottenBeggar, SlotVariant.ROTTEN_BEGGAR)  

function LazarilloMod:updateRottenBeggarSprite()
...
LazarilloMod:AddCallback(ModCallbacks.MC_POST_RENDER , LazarilloMod.updateRottenBeggarSprite)  


function LazarilloMod:clearRottenBeggarData()  
    beingrobbedBeggars = {}
    despawningBeggars = {}
end

LazarilloMod:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, LazarilloMod.clearRottenBeggarData)  
