...
function LazarilloMod:stealFromKeyMaster(entityBeggar, collider)  
    ...
    if math.floor(math.random(0, hitChance)) == 0 then
        ...
        beggarSprite:Load("gfx/beggars/key_master/key_master.anm2", true)  
...
function LazarilloMod:triggerStealFromKeyMaster(entityBeggar, collider, low)	  
    local beggarSprite = entityBeggar:GetSprite()
    ...
    if (not (beggarSprite:IsPlaying("PayPrize") or beggarSprite:IsPlaying("Prize") or beggarSprite:IsPlaying("PayNothing"))) then
        ...
        if playerType == lazaroType then
            ...
            if ((not beggarSprite:IsPlaying("GetStolen")) and notTeleporting)  then
                return LazarilloMod:stealFromKeyMaster(entityBeggar, collider)            
...
LazarilloMod:AddCallback(ModCallbacks.MC_PRE_SLOT_COLLISION, LazarilloMod.triggerStealFromKeyMaster, SlotVariant.KEY_MASTER)  

function LazarilloMod:updateKeyMasterSprite() 
...
LazarilloMod:AddCallback(ModCallbacks.MC_POST_RENDER , LazarilloMod.updateKeyMasterSprite)  


function LazarilloMod:clearKeyMasterData()  
    beingrobbedBeggars = {}
    despawningBeggars = {}
end

LazarilloMod:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, LazarilloMod.clearKeyMasterData)  
