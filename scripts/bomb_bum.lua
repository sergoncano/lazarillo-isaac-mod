...
function LazarilloMod:stealFromBombBum(entityBeggar, collider) 
    ...
    if math.floor(math.random(0, hitChance)) == 0 then
        ...
        beggarSprite:Load("gfx/beggars/bomb_bum/bomb_bum.anm2", true) 
...
function LazarilloMod:triggerStealFromBombBum(entityBeggar, collider, low)	 
    local beggarSprite = entityBeggar:GetSprite()
    ...
    if (not (beggarSprite:IsPlaying("PayPrize") or beggarSprite:IsPlaying("Prize") or beggarSprite:IsPlaying("PayNothing"))) then
        ...
        if playerType == lazaroType then
            ...
            if ((not beggarSprite:IsPlaying("GetStolen")) and notTeleporting)  then
                return LazarilloMod:stealFromBombBum(entityBeggar, collider)           
...
LazarilloMod:AddCallback(ModCallbacks.MC_PRE_SLOT_COLLISION, LazarilloMod.triggerStealFromBombBum, SlotVariant.BOMB_BUM) 

function LazarilloMod:updateBombBumSprite()
    ...
    for _, entityBeggar in ipairs(despawningBeggars) do
        local beggarSprite = entityBeggar:GetSprite(angelChance)        
...
LazarilloMod:AddCallback(ModCallbacks.MC_POST_RENDER , LazarilloMod.updateBombBumSprite) 


function LazarilloMod:clearBombBumData() 
    beingrobbedBeggars = {}
    despawningBeggars = {}
end

LazarilloMod:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, LazarilloMod.clearBombBumData) 
